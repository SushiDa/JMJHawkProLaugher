...
public class Canon : AbstractItem
{
    ...
    void Update()
    {
        ...
        if (spawnTimer >= 5) {
            Tarte.SetActive(true);
            Tarte.transform.position = transform.position;
            Tarte.GetComponent<Rigidbody>().velocity = new Vector3(-7,7,0);
            Tarte.GetComponent<Rigidbody>().rotation = Quaternion.LookRotation(Vector3.back, Vector3.left);
            spawnTimer -= 5;
        }
